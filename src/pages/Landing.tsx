...
export function Landing() {
  ...
  const fetchPackages = async () => {
    try {
      ...
      purchasesData?.forEach((purchase) => {
        purchaseCounts.set(
          purchase.package_id,
          (purchaseCounts.get(purchase.package_id) || 0) + 1
        );
      });

      // Add purchase counts and sort by popularity
      const packagesWithCounts: PackageWithCount[] = (packagesData || [])
        .map((pkg) => ({
          ...pkg,
  ...
  const fetchCredits = async () => {
    try {
      ...
      // Add purchase counts and sort by popularity
      const creditsWithCounts: CreditWithCount[] = (
        (creditsData as DropInCreditPackage[]) || []
      )
        .map((credit) => ({
          ...credit,
    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="animate-fade-in max-w-4xl">
            ...
            <div className="flex gap-4 md:gap-6 justify-center flex-wrap px-4">
              <Link to="/classes" className="w-full sm:w-auto">
                <Button
                  size="lg"
                  className="w-full sm:w-auto bg-white text-purple-900 hover:bg-pink-50 text-lg md:text-xl px-8 md:px-12 py-6 md:py-8 rounded-full shadow-2xl hover:scale-110 transition-all duration-300 font-semibold"
                >
              ...
              {isLoggedIn ? (
                isRegularUser ? (
                  <div className="w-full inline-flex items-stretch border-3 border-white bg-white/20 backdrop-blur-md rounded-full overflow-hidden shadow-2xl hover:scale-105 transition-all duration-300 md:max-w-3xs min-h-12">
                    <Button
                      size="lg"
                      variant="ghost"
                      onClick={() => navigate(getProfileRoute())}
                      className="text-white flex-1 hover:bg-white hover:text-purple-900 text-lg md:text-xl px-6 md:px-10 rounded-l-full rounded-r-none font-semibold border-0 h-auto"
                    >
      ...
      <div className="container mx-auto px-4 py-24">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-gray-500 text-xl">{t("landing.noClasses")}</p>
          </div>
        ) : (
                        ...
                        {cls.schedule_days && cls.schedule_days.length > 0 ? (
                          <>
                            {cls.schedule_days
                              ...
                              .join(", ")}
                            {cls.schedule_time &&
                              ` ${t("admin.at")} ${cls.schedule_time}`}
                            {cls.duration_minutes &&
                              ` (${cls.duration_minutes} ${t(
                                "admin.minutes"
                              )})`}
                          </>
                        ) : (
      ...
      {(packages.length > 0 || credits.length > 0) && (
        <div className="container mx-auto px-4 py-24 bg-gradient-to-b from-white to-pink-50">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10 max-w-7xl mx-auto">
            {/* Class Packages */}
            {(showAllPackages ? packages : packages.slice(0, 3)).map(
              (pkg, index) => {
            ...
            {(showAllPackages
              ? credits
              : credits.slice(0, 3 - Math.min(packages.length, 3))
            ).map((credit, index) => {
              ...
              return (
                <Card
                  ...
                  className={`group hover:shadow-2xl transition-all duration-500 overflow-hidden shadow-xl hover:-translate-y-3 ${
                    isPopular ? "border-4 border-pink-600 relative" : "border-0"
                  }`}
                  style={{
                    animation: `fade-in-up 0.6s ease-out ${
                      displayIndex * 0.1
      ...
      <div className="relative bg-gradient-to-r from-pink-600 via-purple-600 to-pink-600 py-28 overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 text-center">
          ...
          <Link to="/register">
            <Button
              size="lg"
              className="bg-white text-purple-900 hover:bg-pink-50 text-xl px-14 py-8 rounded-full shadow-2xl hover:scale-110 transition-all duration-300 font-bold"
            >
              {t("landing.getStarted")}
