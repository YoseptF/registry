...
export function getSessionsForWeek(classData: Class, weekStart: Date): SessionDate[] {
...
export function getSessionsForMonth(classData: Class, month: Date): SessionDate[] {
...
export function getUpcomingSessions(
  ...
  const upcomingSessions = allSessions.filter((session) => {
    const sessionDateTime = new Date(`${formatSessionDate(session.date)}T${session.time}`)
    return sessionDateTime > new Date()
  })
...
export function groupSessionsByDate(sessions: ClassSession[]): Map<string, ClassSession[]> {
...
export function getPaymentWeekBoundaries(
  paymentDayOfWeek: number = 5
): { start: Date; end: Date } {
  const now = new Date()
